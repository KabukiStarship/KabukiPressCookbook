...
Modern Embedded-C++ (ME-C++) is similar to and can replace Modern-C++ but it is designed for embedded first. Some of the most important characteristics of ME-C++ are:
...
1. Use of a standardized set of general purpose data structures such as ASCII Data Types. **✓**
...
1. Code mostly uses structured programming without Software exceptions with the exception of performance critical applications. **✓**
1. Libraries may integrate `Antiquated-C++`, meaning `C++` code that is not modern, but `Antiquated-C++` is wrapped with `Modern-C++ Adapters` and is segregated from the `Modern-C++` code and marked. **✓**
